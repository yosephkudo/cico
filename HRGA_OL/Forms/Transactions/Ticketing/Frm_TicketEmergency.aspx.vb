Public Class Frm_TicketEmergency
    ...
    Public Sub GetDetail(ByVal s_int_id As Integer, ByVal s_str_TA As String, ByRef s_str_tanggal As String, ByRef s_str_dari As String, ByRef s_str_tujuan As String)
        ...
        With cmd
            .Connection = con
            .CommandType = CommandType.Text
            .CommandText = "SELECT Convert(varchar(10),TglBerangkat,105) Tanggal, Upper(Departure) dari, Upper(Arrive) Tujuan FROM vw_TA_Tiket_onOrder_NonTA WHERE IDFlight=@ID AND TravelAuthorizeNo =@TA"
            .Parameters.Add("@ID", SqlDbType.Int).Value = s_int_id
    ...
    Public Sub Del_AllTemp(ByVal s_str_agent As String)
        ...
        con.Close()
    End Sub
    Public Sub Upd_Temp(ByVal s_int_id As Integer, ByVal s_str_nomor As String, ByVal s_str_agent As String, ByVal s_str_jam As String, ByVal s_str_kelas As String, ByVal s_str_nta As String, ByVal s_str_maskapai As String)
        ...
        With cmd
            .Connection = con
            .CommandType = CommandType.Text
            .CommandText = "UPDATE TBL_TEMP_BOOKING SET JAM=@JAM, KELAS=@KELAS, NTA=@NTA, NOMORTIKET=@NOMORTIKET, AIRLINE=@MASKAPAI WHERE ID=@ID AND HOSTNAME=@HOST  AND ID_AGENT=@AGENT"
            .Parameters.Add("@ID", SqlDbType.Int).Value = s_int_id
            .Parameters.Add("@NOMORTIKET", SqlDbType.VarChar, 20).Value = s_str_nomor
            .Parameters.Add("@HOST", SqlDbType.VarChar, 50).Value = Session("Host")
            .Parameters.Add("@AGENT", SqlDbType.VarChar, 5).Value = s_str_agent
    ...
    Public Function Cek_KodeBooking(ByVal s_str_kodebooking) As Boolean
        ...
        Return Status
    End Function
    Public Sub Exc_Emergency(ByVal s_str_kode As String, ByVal s_str_maskapai As String, ByVal s_flo_nta As String, ByVal s_int_agent As Integer, ByVal s_flo_fee As String)
        ...
        With cmd
            .Connection = con
            .CommandType = CommandType.Text
            .CommandText = "EXECUTE SP_INS_EMERGENCY @KODE,@MASKAPAI,@NTA,@FEE,@AGENT"
    ...
    Private Sub btn_submit_Click(sender As Object, e As EventArgs) Handles btn_submit.Click
        If txt_kodebooking.Text <> "" And txt_kodebooking.Text <> "" And lbl_servicefee.Text > 0 And ddl_jam.SelectedValue <> "" And gv_booking.Rows.Count > 0 Then
            Try
                Dim Tot_NTA As Decimal = 0
                For Each Row As GridViewRow In gv_booking.Rows
                    If Row.RowType = DataControlRowType.DataRow Then
                        Dim ID As Integer = gv_booking.DataKeys(Row.RowIndex).Value.ToString
                        Dim Nomor As String = TryCast(Row.Cells(0).FindControl("txt_NoTiket"), TextBox).Text
                        ...
                        carikoma(NTA)
                        Upd_Temp(ID, Nomor, Session("Agent"), ddl_jam.SelectedValue, Kelas, NTA, Left(ddl_jam.SelectedItem.Text, 2))
                        Tot_NTA = Tot_NTA + CDec(NTA)
                    End If
                Next
                For Each Row As GridViewRow In gv_transit.Rows
                    If Row.RowType = DataControlRowType.DataRow Then
                        ...
                        Dim NTA As String = TryCast(Row.Cells(0).FindControl("txt_NTA"), TextBox).Text
                        carikoma(NTA)
                        Upd_Temp(ID, Nomor, Session("Agent"), ddl_jamtransit.SelectedValue, Kelas, NTA, Left(ddl_jamtransit.SelectedItem.Text, 2))
                        Tot_NTA = Tot_NTA + CDec(NTA)
                    End If
                Next
                For Each Row As GridViewRow In gv_booking_r2.Rows
                    If Row.RowType = DataControlRowType.DataRow Then
                        ...
                        Dim NTA As String = TryCast(Row.Cells(0).FindControl("txt_NTA"), TextBox).Text
                        carikoma(NTA)
                        Upd_Temp(ID, Nomor, Session("Agent"), ddl_jam_r2.SelectedValue, Kelas, NTA, Left(ddl_jam_r2.SelectedItem.Text, 2))
                        Tot_NTA = Tot_NTA + CDec(NTA)
                    End If
                Next
                For Each Row As GridViewRow In gv_transit_R2.Rows
                    If Row.RowType = DataControlRowType.DataRow Then
                        ...
                        Dim NTA As String = TryCast(Row.Cells(0).FindControl("txt_NTA"), TextBox).Text
                        carikoma(NTA)
                        Upd_Temp(ID, Nomor, Session("Agent"), ddl_jamtransit_R2.SelectedValue, Kelas, NTA, Left(ddl_jamtransit_R2.SelectedItem.Text, 2))
                        Tot_NTA = Tot_NTA + CDec(NTA)
                    End If
                Next
                If Tot_NTA = "0" Then
                ...
                Else
                    Exc_Emergency(txt_kodebooking.Text, ddl_maskapai.SelectedValue, Tot_NTA, Session("Agent"), lbl_servicefee.Text)
            ...
            End Try
        Else
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "alertMessage", "alert('Inputan belum lengkap');", True)
        End If
    End Sub
