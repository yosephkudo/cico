...
System.Web.Services.WebMethod()> _
    ...
    Public Sub Getdata_Reservasi(ByVal s_str_nrp As String, ByRef s_str_nama As String, ByRef s_str_telepon As String, ByRef s_str_company As String, ByRef s_str_distrik As String, ByRef s_str_jabatan As String, ByRef s_str_keluarga As String, ByRef s_str_keperluan As String, ByRef s_str_checkin As String, ByRef s_str_checkout As String, ByRef s_str_lokasi As String, ByRef s_str_gedung As String, ByRef s_str_kamar As String, ByRef s_str_notr As String)
        Dim con As SqlClient.SqlConnection = Nothing
        Dim cmd As SqlClient.SqlCommand = Nothing
        cmd = New SqlClient.SqlCommand
        Dim dr As SqlClient.SqlDataReader = Nothing
    ...
    ' Public Sub Ins_Booking(ByVal s_str_nrp As String, ByVal s_str_nama As String, ByVal s_str_hp As String, ByVal s_str_company As String, ByVal s_str_jabatan As String, ByVal s_str_keperluan As String, ByVal s_str_in As String, ByVal s_str_out As String, ByVal s_str_kamar As String, ByVal s_str_site As String, ByVal s_str_notr As String)
    '     Dim con As SqlClient.SqlConnection = Nothing
    '     Dim cmd As SqlClient.SqlCommand = Nothing
    '     cmd = New SqlClient.SqlCommand
    '     Dim connectionStrings As ConnectionStringSettingsCollection = System.Web.Configuration.WebConfigurationManager.ConnectionStrings
    ...
    '         .Connection = con
    '         .CommandType = CommandType.Text
    '         .CommandText = "EXECUTE SP_BOOKING_KAMAR @NRP,@NAMA,@NOHP,@COMPANY,@JABATAN,@KEPERLUAN,@IN,@OUT,@KAMAR,@SITE,@NO_TR"
    '         .Parameters.Add("@NRP", SqlDbType.VarChar, 20).Value = s_str_nrp
    '         .Parameters.Add("@NAMA", SqlDbType.VarChar, 50).Value = s_str_nama
    ...
    Public Sub Ins_Checkin(ByVal s_str_nrp As String, ByVal s_str_nama As String, ByVal s_str_hp As String, ByVal s_str_company As String, ByVal s_str_jabatan As String, ByVal s_str_keperluan As String, ByVal s_str_in As String, ByVal s_str_out As String, ByVal s_str_kamar As String, ByVal s_str_site As String, ByVal s_str_notr As String, ByVal s_str_resep As String)
        Dim con As SqlClient.SqlConnection = Nothing
        Dim cmd As SqlClient.SqlCommand = Nothing
        cmd = New SqlClient.SqlCommand
        Dim connectionStrings As ConnectionStringSettingsCollection = System.Web.Configuration.WebConfigurationManager.ConnectionStrings
        ...
        With cmd
            .Connection = con
            .CommandType = CommandType.Text
            .CommandText = "EXECUTE SP_CHECKIN_KAMAR @NRP,@NAMA,@NOHP,@COMPANY,@JABATAN,@KEPERLUAN,@IN,@OUT,@KAMAR,@SITE,@NO_TR,@RESEP"
            .Parameters.Add("@NRP", SqlDbType.VarChar, 20).Value = s_str_nrp
            .Parameters.Add("@NAMA", SqlDbType.VarChar, 50).Value = s_str_nama
            .Parameters.Add("@NOHP", SqlDbType.VarChar, 20).Value = s_str_hp
            .Parameters.Add("@COMPANY", SqlDbType.VarChar, 20).Value = s_str_company
    ...
    Private Sub txt_nrp_TextChanged(sender As Object, e As EventArgs) Handles txt_nrp.TextChanged
        ...
        Try
            If Not String.IsNullOrEmpty(txt_nrp.Text) Then
                ' Extract NRP from the input text
                Dim inputText As String = txt_nrp.Text.Trim()
                Dim nrp As String = GetNRPFromInput(inputText)  

                ' Validate if the extracted NRP is not empty
                If Not String.IsNullOrEmpty(nrp) Then
                    ' Set the input text to the extracted NRP
                    txt_nrp.Text = nrp
                    Clear()
                    ...
                    Session("NO_TR") = ""
                    Getdata_Reservasi(nrp, txt_nama.Text, txt_nohp.Text, ddl_company.SelectedValue, ddl_distrik.SelectedValue, txt_jabatan.Text, ddl_keluarga.SelectedValue, ddl_keperluan.SelectedValue, txt_tglcheckin.Text, txt_checkout.Text, i_lokasi, i_gedung, i_kamar, Session("NO_TR"))
                    txt_jamcheckin.Text = Format(Now(), "HH:mm")
                    If Not String.IsNullOrEmpty(i_lokasi) Then
                    ...
                    End If
                Else
                    ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "alertMessage", "alert('Invalid input format');", True)
                    ' Clear the input field if the format is invalid
                    txt_nrp.Text = String.Empty
                End If
            End If
        Catch ex As Exception
    ...
    Private Function GetNRPFromInput(inputText As String) As String
        ' Extract NRP from the input text (assuming the format is always "NRP - ...")
        Dim parts As String() = inputText.Split("-"c)
        If parts.Length > 0 Then
            ' Trim and return the first part as NRP
            Return parts(0).Trim()
            Clear()
        Else
            Return ""
    ...
    '             Dim i_gedung As String
    '             Dim i_kamar As String
    '             Getdata_Reservasi(txt_nrp.Text, txt_nama.Text, txt_nohp.Text, ddl_company.SelectedValue, ddl_distrik.SelectedValue, txt_jabatan.Text, ddl_keluarga.SelectedValue, ddl_keperluan.SelectedValue, txt_tglcheckin.Text, txt_checkout.Text, i_lokasi, i_gedung, i_kamar, Session("NO_TR"))
    '             txt_jamcheckin.Text = Format(Now(), "HH:mm")
    '             If i_lokasi <> "" Then
    '                 btn_booking.Visible = False
    '                 ddl_lokasi.SelectedValue = i_lokasi
    ...
    '     If txt_nrp.Text <> "" And txt_nama.Text <> "" And ddl_company.SelectedValue <> "" And ddl_distrik.SelectedValue <> "" And txt_jabatan.Text <> "" And ddl_keluarga.SelectedValue <> "" And ddl_keperluan.SelectedValue <> "" And txt_tglcheckin.Text <> "" And txt_jamcheckin.Text <> "" And txt_checkout.Text <> "" And ddl_kamar.SelectedValue <> "" Then
    ...
    '             Clear()
    '         Else
    '             Ins_Booking(txt_nrp.Text, txt_nama.Text, txt_nohp.Text, ddl_company.SelectedValue, txt_jabatan.Text, ddl_keperluan.SelectedValue, txt_tglcheckin.Text & " " & txt_jamcheckin.Text, txt_checkout.Text, ddl_kamar.SelectedValue, ddl_distrik.SelectedValue, Session("NO_TR"))
    ...
    Private Sub btn_checkin_Click(sender As Object, e As EventArgs) Handles btn_checkin.Click
        If txt_nrp.Text <> "" And txt_nama.Text <> "" And ddl_company.SelectedValue <> "" And ddl_distrik.SelectedValue <> "" And txt_jabatan.Text <> "" And ddl_keluarga.SelectedValue <> "" And ddl_keperluan.SelectedValue <> "" And txt_tglcheckin.Text <> "" And txt_jamcheckin.Text <> "" And txt_checkout.Text <> "" Then
            If ddl_gedung.SelectedItem.Text = "ANTASARI" And Session("GPID") <> "GS ADMIN" And Session("NO_TR") = "" Then
                ...
                Clear()
            Else
                Ins_Checkin(txt_nrp.Text, txt_nama.Text, txt_nohp.Text, ddl_company.SelectedValue, txt_jabatan.Text, ddl_keperluan.SelectedValue, txt_tglcheckin.Text & " " & txt_jamcheckin.Text, txt_checkout.Text, ddl_kamar.SelectedValue, ddl_distrik.SelectedValue, Session("NO_TR"), Session("Nrp"))
    ...
    '     If txt_nrp.Text <> "" And txt_nama.Text <> "" And ddl_company.SelectedValue <> "" And ddl_distrik.SelectedValue <> "" And txt_jabatan.Text <> "" And ddl_keluarga.SelectedValue <> "" And ddl_keperluan.SelectedValue <> "" And txt_tglcheckin.Text <> "" And txt_jamcheckin.Text <> "" And ddl_kamar.SelectedValue <> "" Then
    ...
    '             Clear()
    '         Else
    '             'Updt_Booking(txt_nrp.Text, txt_nama.Text, txt_nohp.Text, ddl_company.SelectedValue, txt_jabatan.Text, ddl_keperluan.SelectedValue, txt_tglcheckin.Text & " " & txt_jamcheckin.Text, txt_checkout.Text, ddl_kamar.SelectedValue, ddl_gedung.SelectedValue, ddl_distrik.SelectedValue, Session("NO_TR"))
